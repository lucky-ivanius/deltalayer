...
export const badRequest = <TContext extends Context, TError extends string>(c: TContext, err?: Partial<HttpErrorResponse<TError>>) => {
  const { error = BAD_REQUEST_ERROR as TError, message, metadata } = err ?? {};

  return c.json<HttpErrorResponse<TError>>({ error, message, metadata }, 400);
};

export const unauthorized = <TContext extends Context, TError extends string>(c: TContext, err?: Partial<HttpErrorResponse<TError>>) => {
  const { error = UNAUTHORIZED_ERROR as TError, message, metadata } = err ?? {};

  return c.json<HttpErrorResponse<TError>>({ error, message, metadata }, 401);
};
...
export const paymentRequired = <TContext extends Context, TError extends string>(c: TContext, err?: Partial<HttpErrorResponse<TError>>) => {
  const { error = PAYMENT_REQUIRED_ERROR as TError, message, metadata } = err ?? {};

  return c.json<HttpErrorResponse<TError>>({ error, message, metadata }, 402);
};

export const forbidden = <TContext extends Context, TError extends string>(c: TContext, err?: Partial<HttpErrorResponse<TError>>) => {
  const { error = FORBIDDEN_ERROR as TError, message, metadata } = err ?? {};

  return c.json<HttpErrorResponse<TError>>({ error, message, metadata }, 403);
};

export const notFound = <TContext extends Context, TError extends string>(c: TContext, err?: Partial<HttpErrorResponse<TError>>) => {
  const { error = NOT_FOUND_ERROR as TError, message, metadata } = err ?? {};

  return c.json<HttpErrorResponse<TError>>({ error, message, metadata }, 404);
};
...
export const methodNotAllowed = <TContext extends Context, TError extends string>(c: TContext, err?: Partial<HttpErrorResponse<TError>>) => {
  const { error = METHOD_NOT_ALLOWED_ERROR as TError, message, metadata } = err ?? {};

  return c.json<HttpErrorResponse<TError>>({ error, message, metadata }, 405);
};

export const unexpectedError = <TContext extends Context>(c: TContext) =>
  c.json<HttpErrorResponse<typeof INTERNAL_SERVER_ERROR>>({ error: INTERNAL_SERVER_ERROR, message: "Unexpected error occurred" }, 500);
